...
def dequantize_4bit_per_row(quant_W: mx.array, scale: mx.array, row_min: mx.array, origin_n_cols: int):
    """
    ...
    Inputs:
        ...
        row_min:  float32, per-row minimum
    ...
    hi = (quant_W >> 4) & mx.uint8(0x0F)               
    lo = quant_W & mx.uint8(0x0F)                      

    
    dequant_W = mx.stack([lo, hi], axis=-1)                     
    dequant_W = dequant_W[:, :origin_n_cols]     ## Dropping the potential padding column                          

    
    scale = scale[:, None]                              
    row_min = row_min[:, None]                          
    W_approx = dequant_W.astype(mx.float32) * scale + row_min  ## Approximation of previous W 
    return W_approx
