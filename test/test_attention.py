...
def test_forward_shapes_and_cache(tiny_config, packed_attn, use_lora_off):
    """The cache holds pre-expansion KV heads, not query heads."""
...
def test_cached_step_matches_the_full_forward(tiny_config, packed_attn, use_lora_off):
    ...
    The full pass takes an explicit causal mask, since incremental decoding is
    causal by construction while an unmasked full forward is bidirectional.
    """
    attn = MistralAttention.from_quantized_weights(tiny_config, packed_attn)
    d = tiny_config.hidden_size_atten
    t = 4
...
def test_masked_positions_are_ignored(tiny_config, packed_attn, use_lora_off):
    """Masking a key position makes the output independent of its content.

    Every position except the perturbed one is unchanged.
    """
    attn = MistralAttention.from_quantized_weights(tiny_config, packed_attn)
...
def test_lora_changes_the_output_once_b_is_nonzero(
    ...
    """lora_B is zero-initialised, so the adapter is a no-op until trained."""
    attn = MistralAttention.from_quantized_weights(tiny_config, packed_attn)
    ...
    attn.q_proj.lora_B.weight = mx.ones_like(attn.q_proj.lora_B.weight)
