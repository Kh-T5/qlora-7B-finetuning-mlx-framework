...
def preprocess_batch(batch):
        ...
        for prompt_ids, response_ids in zip(prompt_enc["input_ids"], response_enc["input_ids"]):
            ids = prompt_ids + response_ids + [eos_id]                  ### Concat prompt, response and end of sentense token for model input
            labels = [-100] * len(prompt_ids) + response_ids + [eos_id] ### labels to keep track of ids belonging to prompt and the ones belonging to response / special tokens
            ...
            attention_mask = [1 if tid != pad_id else 0 for tid in ids] # Mask for padding tokens
